...
让「哪条命令输出什么」有唯一的落点。真正做变换的只有 login 的 workspace
拼装、issue 的字段整形、创建类命令的字段收窄与 label 的 team 字段剥离。

归一化只保证两件事：返回新的顶层容器，且不原地修改传入的响应。嵌套节点仍与
响应共享对象——下游只有 :mod:`linear_cli.output` 读它，深拷贝是白付的成本。
若将来出现会改写数据的消费方，这条边界不成立，需要在此处补 ownership。
"""
