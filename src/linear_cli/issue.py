...
def create(
    ...
    try:
        ...
        emit(normalize.created_issue(issue), output)
...
def view(
    ...
    try:
        ...
        emit(normalize.issue(issue), output)
...
def list_issues(
    ...
    try:
        ...
        emit(normalize.issue_list(issues), output)
...
def update(
    ...
    try:
        ...
        if updated is None:
            emit_not_found_error([f"issue {issue_id!r} 不存在。"])
        emit(normalize.issue(updated), output)
...
def list_comments(
    ...
    try:
        ...
        if comments is None:
            emit_not_found_error([f"issue {issue_id!r} 不存在。"])
        emit(normalize.comment_list(comments), output)
...
def add_comment(
    ...
    try:
        ...
        if comment is None:
            emit_not_found_error([f"issue {issue_id!r} 不存在。"])
        emit(normalize.created_comment(comment), output)
...
def delete_comment_command(
    ...
    try:
        ...
        emit(normalize.deleted_comment(comment_id, deleted), output)
