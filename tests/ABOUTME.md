...
- **命令层**：用 `typer.testing.CliRunner` 进程内调用，断言退出码 + 默认
  JSON 输出（及 `--pretty`）的结构化数据。
...
| `test_issue.py` | `issue create/view/list/update` 测试：必填参数、未登录、未知 Team、GraphQL/HTTP 错误输出、list 的 filter/order-by/include-archived 请求构造、update 的部分更新与名称→UUID 客户端解析（离线）；env 凭据优先于配置文件（离线）；5 条真实 API 测试（create→view round-trip、view 不存在标识、list --limit 1 字段契约、list filters 服务端读回一致、update 全字段 round-trip）凭据由 `real_api.py` 统一注入，缺失时 skip |
